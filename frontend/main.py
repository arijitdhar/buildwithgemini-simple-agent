...
_A2UI_KEYS = ("beginRendering", "surfaceUpdate", "dataModelUpdate", "deleteSurface", "surfaceId")
_TAG_RE = re.compile(r"</?(?:a2a_datapart_json|a2ui-json)>")


def _parse_a2ui_from_text(text: str) -> tuple[list[dict], str]:
    """Extract A2UI message dicts from text containing <a2a_datapart_json>, <a2ui-json>, or raw A2UI JSON.

    Returns (a2ui_messages, remaining_plain_text).
    """
    if not any(k in text for k in _A2UI_KEYS) and "<a2a_datapart_json>" not in text and "<a2ui-json>" not in text:
        return [], text
    ...
    def _add_item(item):
        if isinstance(item, dict):
            if item.get("kind") == "data" and isinstance(item.get("data"), dict):
                item = item["data"]
            if any(k in item for k in ("beginRendering", "surfaceUpdate", "dataModelUpdate", "deleteSurface")):
                a2ui_messages.append(item)
            elif "surfaceId" in item and "components" in item:
                a2ui_messages.append({"surfaceUpdate": item})
    ...
    try:
        ...
        while idx < n:
            ...
            if idx >= n:
                break
            try:
                val, end = decoder.raw_decode(cleaned, idx)
...
def _extract_parts(parts: list) -> list[dict]:
    ...
    for p in parts:
        ...
        if isinstance(root, TextPart) and getattr(root, "text", None):
            text = root.text
            a2ui_msgs, plain = _parse_a2ui_from_text(text)
            for msg in a2ui_msgs:
                out.append({"kind": "a2ui", "data": msg})
            if plain and plain.strip():
                out.append({"kind": "text", "text": plain.strip()})
        elif getattr(root, "data", None) is not None:
            meta = getattr(root, "metadata", None) or {}
            mime = meta.get("mimeType") if isinstance(meta, dict) else None
            data = root.data
            if mime == _A2UI_MIME or (isinstance(data, dict) and any(k in data for k in _A2UI_KEYS)):
                out.append({"kind": "a2ui", "data": data})
            elif isinstance(data, str):
                a2ui_msgs, plain = _parse_a2ui_from_text(data)
                for msg in a2ui_msgs:
                    out.append({"kind": "a2ui", "data": msg})
                if plain and plain.strip():
                    out.append({"kind": "text", "text": plain.strip()})
...
async def chat(req: Request):
    ...
    async with httpx.AsyncClient(headers=_auth_headers(), timeout=120) as client:
        ...
        # Non-streaming fallback: pull parts from the final task's artifacts, history, or status message.
        if not got_artifact_update and last_task is not None:
            for artifact in getattr(last_task, "artifacts", None) or []:
                parts.extend(_extract_parts(artifact.parts))
            if not parts and getattr(last_task, "history", None):
                for msg_item in last_task.history:
                    if getattr(msg_item, "role", None) == Role.agent:
